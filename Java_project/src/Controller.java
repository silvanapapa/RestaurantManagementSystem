...
public class Controller
{
    ...
    {
        Staff staff = cDatabase.findStaffByID(currentUserID);

        if(staff == null)
            return false;
        if(staff.getWorkState() == Staff.WORKSTATE_ACTIVE)
            return true;
        else
            return false;
    }
    ...
    public boolean loginCheck( int inputID, String inputPassword, boolean isManager)
    {
        ...
        Staff staff = cDatabase.findStaffByID(inputID);
        if(isManager)
            searchClassName = "Manager";
        else searchClassName = "Employee";
        if(staff != null)//user found
        {
            if(staff.getClass().getName().equalsIgnoreCase(searchClassName))
            {
                if(staff.getPassword().equals(inputPassword))
                {
                    if(staff.getWorkState() == 0)  //Not logged in yet
                    {
                        staff.clockIn();
                    }
                    if(isManager)
                    ...
                    currentUserName = staff.getFullName();
    ...
    {
        Staff staff = cDatabase.findStaffByID(newID);
        if(staff != null)
        {
            setErrorMessage("ID:" + newID + " is arleady used by " + staff.getFullName());
            return false;
        }
    ...
    {
        Staff staff = cDatabase.findStaffByID(id);
        if(staff == null)
        {
    ...
    {
        Staff staff = cDatabase.findStaffByID(staffID);

        byte state = staff.getWorkState();
        ...
        switch(state)
        {
            ...
            case Staff.WORKSTATE_FINISH:
                setErrorMessage("Staff:" + staff.getFullName() + " already clocked out.");
                break;
            default:
                setErrorMessage("Staff:" + staff.getFullName() + "has not been on work today.");
                break;
        }
    ...
    {
        Item item_menu = cDatabase.findMenuItemByID(newID);
        if(item_menu != null)
        {
            setErrorMessage("ID:" + newID + " is arleady used by " + item_menu.getName());
            return false;
        }
    ...
    {
        Item rMenuItem= cDatabase.findMenuItemByID(id);
        if(rMenuItem == null)
        {
    ...
    public Item getMenuItemData(int menuItemID)
    {
    ...
    public boolean addNewOrderItem(int orderID, int addItemID, byte addItemQuantity)
    {
        ...
        {
            setErrorMessage("You cannot edit the order.\n(The waiter for this order is " + rOrder.getStaffName() + ")");
            return false;
        }

        Item NewItem = null;

        NewItem = cDatabase.findMenuItemByID(addItemID);
        if(NewItem == null)
        {
        ...
        cDatabase.addOrderItem(orderID, NewItem, addItemQuantity);
        return true;
    }
    ...
    public boolean deleteOrderItem(int orderID, int deleteNo)
    {
        ...
        if(currentUserID != rOrder.getStaffID())
        {
            setErrorMessage("You cannot delete this order item.\n(The waiter for this order is " + rOrder.getStaffName() + ")");
            return false;
        }
    ...
    public boolean closeOrder(int closeOrderID)
    {
        ...
        if( currentUserID != rOrder.getStaffID())
        {
            setErrorMessage("You cannot close this order.\n(The waiter for this order is " + rOrder.getStaffName() + ")");
            return false;
        }
    ...
    public boolean cancelOrder(int cancelOrderID)
    {
        ...
        if( currentUserID != rOrder.getStaffID())
        {
            setErrorMessage("You cannot cancel this order.\n(The waiter for this order is " + rOrder.getStaffName() + ")");
            return false;
        }
    ...
    {
        Iterator<Item> it = cDatabase.getMenuList().iterator();
        ArrayList<String> initData = new ArrayList<String>();
        ...
        while (it.hasNext()) {
            ...
            if(re.getState() == Item.PROMOTION_ITEM)
            {
