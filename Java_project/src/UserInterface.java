...
public class UserInterface extends JFrame implements ActionListener
{
    ...
    public UserInterface(Controller rController)
    {
        ...
        // Set frame
        setTitle("L'Osteria Restaurant Management System");
        ...
        homeImage.setIcon(new ImageIcon("image/fundal.jpg"));
    ...
    private void createMasterPanelConpornents()
    {
        ...
        headTitle = new JLabel("L'Osteria Restaurant");
        ...
        taMessage.setText("Welcome! Have a great shift!");
    ...
    private class MenuListPanel extends JPanel implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            {
                showMenuList(Item.MAIN);
            }
            else if (ae.getSource() == btnDrink)
            {
                showMenuList(Item.DRINK);
            }
            else if (ae.getSource() == btnAlcohol)
            {
                showMenuList(Item.ALCOHOL);
            }
            else if (ae.getSource() == btnDessert)
            {
                showMenuList(Item.DESSERT);
            }
        }
    }
    ...
    private class EditMenuItemPanel extends JPanel implements ActionListener
    {
        ...
        public void init(int menuItemID)
        {
            ...
            Item   rMenuItem = rcController.getMenuItemData(menuItemID);
            isUpdate = true;
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == btnOK)
            {
                ...
                {
                    menuType = Item.ALCOHOL;
                }
                else
                {
                    menuType = Item.DESSERT;
                }
    ...
    private class OrderDetailPanel extends JPanel implements ActionListener, ListSelectionListener
    {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            {
                menuList.setListData(rcController.createMenuList(Item.MAIN).toArray());
                menuScrollPanel.getViewport().setView(menuList);
            }
            else if (ae.getSource() == btnDrink)
            {
                menuList.setListData(rcController.createMenuList(Item.DRINK).toArray());
                menuScrollPanel.getViewport().setView(menuList);
            }
            else if (ae.getSource() == btnAlcohol)
            {
                menuList.setListData(rcController.createMenuList(Item.ALCOHOL).toArray());
                menuScrollPanel.getViewport().setView(menuList);
            }
            else if (ae.getSource() == btnDessert)
            {
                menuList.setListData(rcController.createMenuList(Item.DESSERT).toArray());
                menuScrollPanel.getViewport().setView(menuList);
            }
        }
